...
validate.classificationRules = () => {
  ...
  validate.inventoryRules = () => {
    return [
      ...
      body("inv_model")
        .custom((value, { req }) => {
          if (!alphanumericRegex.test(value)) {
            throw new Error("Please provide a car model with only letters, numbers, and no spaces.");
          }
          return true;
        }),
      ...
      body("inv_description")
        .custom((value, { req }) => {
          if (!alphanumericWithSpacesRegex.test(value)) {
            throw new Error("Please provide a description with only letters, numbers, and spaces.");
          }
          return true;
        }),
      ...
      body("inv_image")
        .custom((value, { req }) => {
          if (!alphanumericRegex.test(value)) {
            throw new Error("Please provide a valid image path with only letters, numbers, and no spaces.");
          }
          return true;
        }),
      ...
      body("inv_thumbnail")
        .custom((value, { req }) => {
          if (!alphanumericRegex.test(value)) {
            throw new Error("Please provide a valid thumbnail image path with only letters, numbers, and no spaces.");
          }
          return true;
        }),
      ...
      body("inv_price")
        .isNumeric({ min: 1 })
        .withMessage("Please provide a valid car price with no spaces"),
  
      body("inv_miles")
        .isNumeric({ min: 1 })
        .withMessage("Please provide a valid car mileage with no spaces"),
  ...
  validate.checkInventoryData = async (req, res, next) => {
    const {inv_make, inv_model, inv_year, inv_description, inv_image, inv_thumbnail, inv_price, inv_mileage, inv_color, classification_id } = req.body
    let errors = []
